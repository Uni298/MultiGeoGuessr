...
function broadcastRoomUpdate(roomId){
  ...
  if(!room) return;
  const players = Object.values(room.players).map(p=>({id:p.id,name:p.name,color:p.color,connected:p.connected, submitCount: p.submitCount || 0, excluded: p.excluded||false}));
...
io.on('connection', (socket) => {
  ...
  function endRound(roomId){
    ...
    const results = Object.values(room.players).map(p => ({id:p.id, name:p.name, color:p.color, lastGuess:p.lastGuess, excluded: p.lastGuess? false : true, submitCount: p.submitCount||0}));
    room.status = 'ended';
