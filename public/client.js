...
socket.on('round_ended', async ({ results, spawnIndex }) => {
  ...
  results.forEach(r => {
    if (r.lastGuess) {
      // store marker data to create later
      delayedMarkers.push({ lat: r.lastGuess.lat, lng: r.lastGuess.lng, color: r.color || '#333' });
      // store lines to draw later (after map zoom/resize) - use dashArray for dashed lines
      delayedLines.push({ from: [spawn.lat, spawn.lng], to: [r.lastGuess.lat, r.lastGuess.lng], color: r.color || '#2563eb' });
      bounds.push([r.lastGuess.lat, r.lastGuess.lng]);
      bounds.push([spawn.lat, spawn.lng]);
    }
  });

  // build ranking (exclude players with excluded=true)
  const scored = results.map(r => {
    if (!r.lastGuess) return Object.assign({}, r, { distance: null, score: 0 });
    const d = calcDistance(spawn.lat, spawn.lng, r.lastGuess.lat, r.lastGuess.lng);
    return Object.assign({}, r, { distance: d });
  });
  const ranked = scored.filter(s => s.distance !== null).sort((a, b) => a.distance - b.distance).slice(0, 10);
  // display ranking in side list
  const rankEl = $('ranking-list'); rankEl.innerHTML = '';
  ranked.forEach((r, i) => { const li = document.createElement('li'); li.textContent = `${i + 1}. ${r.name} — ${r.distance.toFixed(2)} km`; rankEl.appendChild(li); });
